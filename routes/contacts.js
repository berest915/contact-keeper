...
router.post('/',
    ...
    async (req, res) => {
        ...
        if (isNameExist) {
            return res.status(400).json({ msg: 'unable to add:: name exactly same with existing contacts' })
        }
...
router.put('/:id',
    ...
    async (req, res) => {
        ...
        if (!mongoose.Types.ObjectId.isValid(req.params.id)) {
            return res.status(400).json({ msg: 'params id not in valid format with _id' })
        }
